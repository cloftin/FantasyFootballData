...
#'@export
download_pictures <- function(year = 2022) {
  ...
  while(!lastPage) {
    ...
    url <- paste0("https://fantasy.nfl.com/research/projections?offset=", (25*(p-1) + 1), "&position=O&sort=projectedPts&statCategory=projectedStats&statSeason=2022&statType=seasonProjectedStats#researchProjections=researchProjections%2C%2Fresearch%2Fprojections%253Foffset%253D", (25*(p-1) + 1), "%2526position%253DO%2526sort%253DprojectedPts%2526statCategory%253DprojectedStats%2526statSeason%253D2022%2526statType%253DseasonProjectedStats%2526statWeek%253D1%2Creplace")
  ...
  for(i in 1:nrow(dat)) {
    ...
    e <- tryCatch({download.file(piclink, destfile = paste0("www/", dat$name[i], ".jpg"),
